...
class Roost(BaseModelClass):
    ...
    def __init__(
        ...
        **kwargs
        ...
        #     ResidualNetwork(out_hidden[0], n, out_hidden[1:], return_features=True) for n in n_targets
        # ])
        self.output_nn = ResidualNetwork(out_hidden[0], n_targets[0], out_hidden[1:], return_features=True)
        self.final = nn.Linear(out_hidden[-1], n_targets[0])
    ...
    def forward(self, elem_weights, elem_fea, self_fea_idx, nbr_fea_idx, cry_elem_idx):
        ...
        # print(f'crystal feature shape: {crys_fea.shape} ')
        # print(f'elem_fea feature shape: {elem_fea.shape} ')
        # print(f'nbr_fea_idx feature shape: {nbr_fea_idx.shape} ')
        # import sys
        # sys.exit()
        crys_fea_0 = F.relu(self.trunk_nn(crys_fea))
        # print(crys_fea.shape)
        # import sys
        # sys.exit()
        crys_fea = self.output_nn(crys_fea_0)
        # print(type(crys_fea))
        # print(crys_fea.shape)
        output = self.final(crys_fea)
        return [output], crys_fea_0

        # apply neural network to map from learned features to target
        # return (output_nn(crys_fea) for output_nn in self.output_nns), crys_fea
...
class MessageLayer(nn.Module):
    ...
    def __init__(self, elem_fea_len, elem_heads, elem_gate, elem_msg):
        ...
        self.pooling = nn.ModuleList(
            [
                WeightedAttentionPooling(
                    # gate_nn=SimpleNetwork(2 * elem_fea_len, 1, elem_gate, batchnorm=True),
                    # message_nn=SimpleNetwork(2 * elem_fea_len, elem_fea_len, elem_msg, batchnorm=True),
