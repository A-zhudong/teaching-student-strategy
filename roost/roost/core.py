...
class BaseModelClass(nn.Module, ABC):
    ...
    def fit(  # noqa: C901
        ...
        # if  self.best_val_scores is None:
        #     best_val_scores = 
        try:
            for epoch in range(start_epoch, start_epoch + epochs):
                ...
                if checkpoint:
                    ...
                    # the multi-task case?
                    save_checkpoint(checkpoint_dict, is_best[0], model_name, run_id)
    ...
    def evaluate(
        ...
        for inputs, targets, formula, *_ in tqdm(
            ...
            if embedding_loss:
                ...
                if torch.is_tensor(formula_embeddings[formula[0]]):
                    batch_embedding = [formula_embeddings[formu] for formu in formula]
                else:
                    batch_embedding = [torch.from_numpy(self.csv_atoms[formu]).unsqueeze(0) for formu in formula]
                embs_alignn = torch.cat(batch_embedding, dim=0).to(embddings.device)
                # self.Norm = torch.nn.LayerNorm(256).to(embs_alignn.device)
                # embs_alignn = self.Norm(embs_alignn)
                # print(embddings.shape, embs_alignn.shape)
                loss_emb = criterion_emb(embddings, embs_alignn)         
                alpha = 0.01       # default 6
                beta = 1
                if not finetune:
                    beta = 0   
                print(mixed_loss, loss_emb)
                mixed_loss = (mixed_loss*beta + loss_emb*alpha)/(beta+alpha)
