'''
this is the model structure for the two models devide and conquer mode
devide the model to the property and structure predictors
'''
...
class CrabNet_ori(nn.Module):
    ...
    def forward(self, src, frac, crab_ori=None):
        # with torch.no_grad():
    ...
    #     output = output.sum(dim=1)/(~mask).sum(dim=1)  # B*N_elements*3 average elements
    #     output, logits = output.chunk(2, dim=-1)
    #     # print('logits:', logits.shape, 'output: ', output.shape)
    ...
    #     output = output * probability
        if self.avg:
            ...
            output = output.masked_fill(mask, 0)
            if self.pretrained:
                concat = (output + self.src_nn(x_src)).masked_fill(mask, 0)
                concat = self.res_nn(concat)
                embeddings = concat.sum(dim=1)/(~mask).sum(dim=1)  # B*N_elements*3 average elements
            else:
                embeddings = output.sum(dim=1)/(~mask).sum(dim=1)  # B*N_elements*3 average elements
            
            if crab_ori is not None:
                # print(embeddings.shape, crab_ori.shape)
                embeddings = embeddings+crab_ori
            
            output = self.final(embeddings)
    
        return output, embeddings

class CrabNet_twoWay(nn.Module):
    def __init__(self,
                compute_device=None,
                pretrained=False,
                out_dims=256,
                ...
                N=3,
                heads=4,
                path_left = '/home/zd/zd/teaching_net/CrabNet/models/jarvis_e_form_models/pretrained_models/jarvis_e_form_pretrain_epoch299.pth',
                path_right = '/home/zd/zd/teaching_net/CrabNet/predict_structurEmbedding/jarvis_e_form/models/jarvis_e_form_struPred_epoch299.pth',):
        super().__init__()
        self.compute_device = compute_device
        self.out_dims = out_dims
        self.d_model = d_model
        self.N = N
        self.heads = heads
        self.crab_left_ori = CrabNet_ori(compute_device=compute_device, pretrained=pretrained)
        self.crab_right_stru = CrabNet_ori(compute_device=compute_device, pretrained=pretrained)
        self.crab_left_ori.load_state_dict(torch.load(path_left)['weights'], strict=False)
        self.crab_right_stru.load_state_dict(torch.load(path_right)['weights'], strict=False)
        self.concat_final = torch.nn.Linear(256, 1)

    def forward(self, src, frac):
        out_left_ori, emb_left_ori = self.crab_left_ori(src, frac)
        out_right_str, emb_right_stru = self.crab_right_stru(src, frac)
        return self.concat_final(emb_left_ori+emb_right_stru),  emb_left_ori+emb_right_stru


# %%
if __name__ == '__main__':
    model = CrabNet_twoWay()
