...
class CrabNet(nn.Module):
    def __init__(self,
        ...
        self.final = torch.nn.Linear(32,1)
    ...
    def forward(self, src, frac, crab_ori=None):
        ...
        mask_1 = (src == 0).unsqueeze(-1).repeat(1, 1, 32)
    ...
    #     output = output * probability
        if self.avg:
            ...
            if self.pretrained:
                ...
                # embeddings = concat.sum(dim=1)/(~mask).sum(dim=1)
                embeddings = concat.sum(dim=1)/(~mask_1).sum(dim=1)  # B*N_elements*3 average elements
            else:
